...
class LineDetect:
    def __init__(self):
        ...
        self.linear = 0.3
        self.PID_init()
        self.last_move_time = time.time()  # Track the last time the robot moved
        self.shutdown_complete = False
        if self.CameraDevice == "Astra": img_topic = "/camera/rgb/image_raw"
        else: img_topic = "/camera/color/image_raw"
        if self.VideoSwitch == False:
            from cv_bridge import CvBridge
            self.bridge = CvBridge()
            self.sub_img = rospy.Subscriber(img_topic, Image, self.image_callback)
    ...
    def image_callback(self, msg):
        ...
        self.check_stationary()  # Check if the robot is stationary

    def check_stationary(self):
        # Check if the" robot has been stationary for more than 2 seconds
        if self.Start_state and time.time() - self.last_move_time > 2:
    ...
    def shutdown_and_launch(self):
        ...
        print("Robot has been stationary for 2 seconds. Restarting nodes...")
        subprocess.Popen(["gnome-terminal", "--","bash","-c", "roslaunch transbot_laser laser_Avoidance.launch; exec bash"])
    ...
    def execute(self, point_x, color_radius):
        ...
        # Update the last move time if the robot is moving
